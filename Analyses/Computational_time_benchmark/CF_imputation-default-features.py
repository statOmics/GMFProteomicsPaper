...
for x in [100, 1000, 10000, "1e+05"]:
    fn_intensities = ('../../Data/Computational_time_benchmark/simulated_' + str(x) + '_features_' + str(fixed_sample) + '_samples_nocolnames.csv')
    ...
    df_imputed = model.transform(series)
    ...
    with open('Output/time_measurement_CF_default_' + str(x) + '_features_' + str(fixed_sample) + '.txt', 'w') as file:
        file.write(f"Elapsed time: {elapsed_time} seconds\n")

    
    df_imputed.to_csv('Output/CF_default_' + str(x) + '_features_' + str(fixed_sample) + '_samples.csv')
