...
for(i in c("25","50","75")){
  ...
  original <- readRDS(file =  here("Data","CPTAC_nolab1", "CPTAC_sce.RDS"))
  NAs <- readRDS(file  =  here("Data","CPTAC_nolab1", paste0("CPTAC_MISTOT10_MNAR", i, "_", j, ".RDS")))
  original <- original[rowMeans(is.na(NAs$data)) < 1 ,]
  
  sgd <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output", 
                             paste0("sgd_MISTOT10_MNAR", i, "_", j, ".RDS")))
  
  sgd_batch <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output", 
  ...
  DAE <- as.matrix(data.table::fread(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("DAE_MISTOT10_MNAR", i, "_", j,".csv")),
                  header = "auto"), rownames = 1)
  
  DAE_default <- as.matrix(data.table::fread(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("DAE_default_MISTOT10_MNAR", i, "_", j,".csv")),
                  header = "auto"), rownames = 1)
  
  VAE <- as.matrix(data.table::fread(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("VAE_MISTOT10_MNAR", i, "_", j,".csv")),
                  header = "auto"), rownames = 1)
  
  VAE_default <- as.matrix(data.table::fread(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("VAE_default_MISTOT10_MNAR", i, "_", j,".csv")),
                  header = "auto"), rownames = 1)
  
  CF <- as.matrix(data.table::fread(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("CF_MISTOT10_MNAR", i, "_", j,".csv")),
                  header = "auto"), rownames = 1)
  
  CF_default <- as.matrix(data.table::fread(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("CF_default_MISTOT10_MNAR", i, "_", j,".csv")),
                  header = "auto"), rownames = 1)
  
    nipals <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output",
                              paste0("nipals_MISTOT10_MNAR", i, "_",j, ".RDS")))
  nipals_reconstructed <- reconstruct_nipals(nipals)
  
  knnmethod <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("knnmethod_MISTOT10_MNAR", i, "_", j, ".RDS")))
  
  qrilc <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("qrilc_MISTOT10_MNAR", i, "_", j, ".RDS")))

  zero <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output", 
                              paste0("zero_MISTOT10_MNAR", i, "_", j, ".RDS")))
  minimum <- readRDS(file = here("Analyses","CPTAC_nolab1", "Output",  
...
df_all$method <- factor(df_all$method, 
                     ...
                     "DAE-NN","VAE-NN", "CF-NN",
                     "DAE-NN-default","VAE-NN-default", "CF-NN-default",
                     "NIPALS", "BPCA","knnmethod","QRILC", "zero","minimum"))
saveRDS(object = df_all, file = here("Analyses","CPTAC_nolab1", "Output", "imputation_results_CPTAC_nolab1.RDS"))
```
