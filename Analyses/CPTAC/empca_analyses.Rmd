...
```{r}
#Copied the function from the nipals package.
...
# Use empca compute the principal components on the original data without imputed values.

df <- empca(data, center = FALSE, scale = FALSE, ncomp = 6, 
...
saveRDS(object = df, file = paste0(here("Analyses/CPTAC/Output/"),"empca_sce.RDS"))
saveRDS(object = time_difference, 
        file = paste0(here("Analyses/CPTAC/Output/"),"empca_time.RDS"))

```

```{r}

# Use empca to impute missing values in the simulations for 10% introduced missing values
# of which 25% are missing due to low abundance, generated in /Data/CPTAC_data.Rmd.
...
for(i in c(1:10)){
  ...
  df <- empca(data, center = FALSE,  scale = F, ncomp = 6, 
               fitted = TRUE, verbose = TRUE)
    
  saveRDS(object = df, file = paste0(here("Analyses","CPTAC","Output"),"/empca_MISTOT10_MNAR25_", i, ".RDS"))
...
set.seed(100)
# Use empca to impute missing values in the simulations for 10% introduced missing values
# of which 50% are missing due to low abundance, generated in /Data/CPTAC_data.Rmd.
...
for(i in c(1:10)){
  ...
  saveRDS(object = df, file = paste0(here("Analyses","CPTAC","Output"),"/empca_MISTOT10_MNAR50_", i, ".RDS"))
...
set.seed(100)
# Use empca to impute missing values in the simulations for 10% introduced missing values
# of which 75% are missing due to low abundance, generated in /Data/CPTAC_data.Rmd.
...
for(i in c(1:10)){
  ...
  saveRDS(object = df, file = paste0(here("Analyses","CPTAC","Output"),"/empca_MISTOT10_MNAR75_", i, ".RDS"))
