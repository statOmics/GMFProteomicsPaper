...
model = CollaborativeFilteringTransformer(
    ...
    n_factors=3, # dimension of separate sample and item embedding
    batch_size = 4096
)
...
df_imputed.to_csv("Output/CF_sce_3LF.csv")
...
for x in range(1,11):
    ...
    df_imputed.to_csv("Output/CF_MISTOT10_MNAR25_3LF_" + str(x) + ".csv")
...
for x in range(1,11):
    ...
    df_imputed.to_csv("Output/CF_MISTOT10_MNAR50_3LF_" + str(x) + ".csv")
...
for x in range(1,11):
    ...
    df_imputed.to_csv("Output/CF_MISTOT10_MNAR75_3LF_" + str(x) + ".csv")
