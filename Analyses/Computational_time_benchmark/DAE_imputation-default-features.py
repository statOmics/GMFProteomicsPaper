...
for x in [100, 1000, 10000, "1e+05"]:
    ...
    with open('Output/time_measurement_DAE_default_' + str(x) + '_features_' + str(fixed_sample) + '.txt', 'w') as file:
        file.write(f"Elapsed time: {elapsed_time} seconds\n")

    
    df_imputed.to_csv('Output/DAE_default_' + str(x) + '_features_' + str(fixed_sample) + '_samples.csv')
