...
myColors <- c("#BC3C29FF",  "#E18727FF", "#E18727FF",
              ...
              "black")
names(myColors) <- c("SGD", "SGD Batch", "SGD Treatment", 
                     "DAE", "VAE", "CF",
                     "DAE-CV","VAE-CV", "CF-CV",
                     "NIPALS",
...
titles <- c("SGD", "DAE","VAE","CF","NIPALS","KNN","QRILC","zero","minimum")
...
```{r}
DAE_PC <- prcomp(DAE, rank = 8)
VAE_PC <- prcomp(VAE, rank = 8)
CF_PC <- prcomp(CF, rank = 8)

KNN_PC <- prcomp(KNN, rank = 8)
QRILC_PC <- prcomp(QRILC, rank = 8)
zero_PC <- prcomp(zero, rank = 8)
minimum_PC <- prcomp(minimum, rank = 8)
...
df_cor <- data.frame("correlation" = c(cor_sgd, cor_DAE, cor_VAE, cor_CF, cor_nipals, cor_knn, cor_qrilc, cor_zero, cor_minimum,
                     ...
                     "method" = rep(c("SGD","DAE","VAE", "CF", "NIPALS", "KNN", "QRILC","zero","minimum"), times = 4),
                     "PC" = rep(c("PC1","PC2"), each = 9),
                     "cormethod" = c(rep(c("Pearson", "Spearman"), each = 18)))
df_cor$method <- factor(df_cor$method, levels = c("SGD","DAE","VAE", "CF", "NIPALS", "KNN", "QRILC","zero","minimum"))

plt1Panel2 <- ggplot(df_cor %>% filter(method %in% c("SGD", "DAE","VAE", "CF", "NIPALS", "KNN")), 
...
titles <- c("SGD Batch", "DAE","VAE","CF","NIPALS","KNN","QRILC","zero","minimum")
...
df_summarised_Leduc$method <- factor(df_summarised_Leduc$method, levels = c("SGD","SGD Mean shift","SGD Batch", "SGD Batch Mean shift",
                                              "DAE-NN", "DAE-NN-default", 
                                              "VAE-NN", "VAE-NN-default",
                                              "CF-NN", "CF-NN-default",
                                              "NIPALS", "knnmethod", 
                                              "BPCA","QRILC","zero","minimum"), 
                                     labels = c("SGD","SGD Mean shift","SGD Batch", "SGD Batch Mean shift",
                                              "DAE-CV", "DAE", 
                                              "VAE-CV", "VAE",
                                              "CF-CV", "CF",
                                              "NIPALS", "KNN", 
...
```{r}
Petrosius_imputation_plot <- ggplot(df_summarised_Petrosius %>% filter(method %in% c("SGD","SGD Treatment", 
                                              "DAE", "VAE", "CF", 
                                              "NIPALS", "KNN")), 
...
Leduc_imputation_plot <- ggplot(df_summarised_Leduc %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                              "NIPALS" ,"KNN")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
```{r}
Petrosius_imputation_plot_CV <- ggplot(df_summarised_Petrosius %>% filter(method %in% c("SGD","SGD Treatment", 
                                              "DAE", "VAE", "CF", 
                                              "DAE-CV", "VAE-CV", "CF-CV")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
Leduc_imputation_plot_CV <- ggplot(df_summarised_Leduc %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                              "DAE-CV", "VAE-CV", "CF-CV")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
```{r}
Petrosius_imputation_plot_all <- ggplot(df_summarised_Petrosius %>% filter(method %in% c("SGD", "SGD Treatment",  
                      "DAE", "VAE", "CF", 
                     "NIPALS",
                     "KNN", "QRILC","zero","minimum")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
...
Leduc_imputation_plot_all <- ggplot(df_summarised_Leduc %>% filter(method %in% c("SGD", "SGD Batch",  
                      "DAE", "VAE", "CF", 
                     "NIPALS",
                     "KNN", "QRILC","zero","minimum")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
...
```{r}
CPTAC_all_labs_imputation_plot <- ggplot(df_summarised_CPTAC_all %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                               "KNN")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
CPTAC_all_labs_imputation_plot_all <- ggplot(df_summarised_CPTAC_all %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                               "KNN", "QRILC","zero","minimum")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
CPTAC_nolab1_imputation_plot <- ggplot(df_summarised_CPTAC_nolab1 %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                              "KNN")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
CPTAC_nolab1_imputation_plot_all <- ggplot(df_summarised_CPTAC_nolab1 %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
...
CPTAC_only_lab1_imputation_plot <- ggplot(df_summarised_CPTAC_lab1 %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                              "KNN")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
CPTAC_only_lab1_imputation_plot_all <- ggplot(df_summarised_CPTAC_lab1 %>% filter(method %in% c("SGD","SGD Batch", 
                                              "DAE", "VAE", "CF", 
                                              "KNN", "QRILC","zero","minimum")), 
       aes(x = method, y = meanMAE, col = method, shape = missing_type)) + 
  geom_point(size = 2.5) + 
...
data_all$origin <- factor(rep(c("original", "Imputed_SGD","DAE","VAE", "CF", "KNN"), 
                        each = nrow(original_assay)), 
                        levels = c("original", "Imputed_SGD","DAE","VAE", "CF","KNN"),
                        labels = c("Observed", "SGD-imputed","DAE-imputed","VAE-imputed", "CF-imputed","KNN-imputed"))
...
Imputation_distribution_UPS_nolab1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(((missing == TRUE)|(missing == FALSE & origin %in% c("Observed"))) & !(origin %in% c("SGD_reconstructed"))), 
...
Imputation_distribution_nonUPS_nolab1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][!grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(((missing == TRUE)|(missing == FALSE & origin %in% c("Observed"))) & !(origin %in% c("SGD_reconstructed"))), 
  aes(x = condition, y = intensity, col = condition)) + 
  geom_violin() + 
  facet_wrap(~origin, ncol = 3) +
  labs(col = "Condition") + 
...
Imputation_Figure <- 
  ggarrange(
    ...
    Imputation_distribution_UPS_nolab1_final, nrow = 2,
...
```{r}
imputation_plot_per_lab_UPS <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter((missing == TRUE & (origin %in% c("SGD-imputed", "CF-imputed")) | (origin =="Observed"))), 
  aes(x = condition, y = intensity, col = condition)) + 
  geom_violin() + 
...
imputation_plot_per_lab_nonUPS <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][!grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter((missing == TRUE & (origin %in% c("SGD-imputed", "CF-imputed")) | (origin =="Observed"))), 
  aes(x = condition, y = intensity, col = condition)) + 
  geom_violin() + 
  facet_wrap(~lab*origin, ncol = 3) +
  theme_bw() + 
...
```{r}
cv_plot <- ggplot(data = df_cv_all,
	       aes(x = ncomp, y = deviance, col = method)) +
	  geom_point() + 
	  geom_line() + 
	  theme_bw() + 
	  ylab("Mean out-of-sample deviances") + 
...
ggsave(plot = cv_plot, filename = here("Figures/cv_plot.pdf"), device = "pdf", dpi = 500, units = "mm", width = 180, height = 90)
```
