...
plot_CPTAC_all[[1]] <- plot_CPTAC_all[[1]] + xlab("GMF 1") + ylab("GMF 2")
...
```{r}
names(MF) <- titles
excel_supp_fig_9 <- bind_rows(
  lapply(names(MF), function(n) {
    df <- MF[[n]][ , c("PC1", "PC2")] %>% 
      as.data.frame() %>% 
      mutate("Condition" = colData(sce_CPTAC)$condition,
             "lab" = lab,
             "method" = n)
    df
  })
...
plot_CPTAC[[1]] <- plot_CPTAC[[1]] + xlab("GMF 1") + ylab("GMF 2")
...
```{r}
names(MF) <- titles
excel_supp_fig_11 <- bind_rows(
  lapply(names(MF), function(n) {
    df <- MF[[n]][ , c("PC1", "PC2")] %>% 
      as.data.frame() %>% 
      mutate("Condition" = colData(sce_CPTAC_nolab1)$condition,
             "lab" = colData(sce_CPTAC_nolab1)$lab,
             "method" = n)
    df
  })
)
...
```{r}
CPTAC_pca_plot_missing <- lapply(1:9, function(i){
  ggplot(data = MF[[i]], 
       aes(x = PC1,y = PC2, col = colMeans(is.na(assay(sce_CPTAC_nolab1))))) +
  geom_point(alpha = 0.5)+ scale_colour_gradient2(name = "Proportion \nmissing",
  low = ("red"),
  mid = "grey",
  high = ("blue"),
  midpoint = median(colMeans(is.na(assay(sce_CPTAC_nolab1)))),
  space = "Lab",
  na.value = "grey50",
  transform = "identity",
  guide = "colourbar",
...
titles <- c("GMF", "DAE","VAE","CF","NIPALS","KNN","QRILC","zero","minimum")
MF <- list(sgd, DAE_PC$x, VAE_PC$x, CF_PC$x, nipals$scores, KNN_PC$x, 
           QRILC_PC$x, zero_PC$x, minimum_PC$x)
...
Petrosius_pca_plot_sampletype[[1]] <- Petrosius_pca_plot_sampletype[[1]] + xlab("GMF 1") + ylab("GMF 2")
#BPCA not converged
...
names(MF) <- titles
excel_supp_fig_2 <- bind_rows(
  lapply(names(MF), function(n) {
    df <- MF[[n]][ , c("PC1", "PC2")] %>% 
      as.data.frame() %>% 
      mutate("Cell Type" = colData(sce_Petrosius)$SampleType,
             "method" = n)
    df
  })
)
...
supp_fig_15 <- list("GMF" = MF[[1]], "GMF Treatment" = sgd_batch)
excel_supp_fig_15 <- bind_rows(
  lapply(names(supp_fig_15), function(n) {
    df <- supp_fig_15[[n]][ , c("PC1", "PC2")] %>% 
      as.data.frame() %>% 
      mutate("Celltype" = colData(sce_Petrosius)$SampleType,
             "method" = n)
    df
  })
)
...
Petrosius_pca_plot_missing <- lapply(1:9, function(i){
  ggplot(data = MF[[i]], 
       aes(x = PC1,y = PC2, col = colMeans(is.na(assay(sce_Petrosius))))) +
  geom_point(alpha = 0.5)+ scale_colour_gradient2(name = "Proportion \nmissing",
  low = ("red"),
  mid = "grey",
  high = ("blue"),
  midpoint = median(colMeans(is.na(assay(sce_Petrosius)))),
  space = "Lab",
  na.value = "grey50",
  transform = "identity",
  guide = "colourbar",
...
Petrosius_pca_plot_missing[[1]] <- Petrosius_pca_plot_missing[[1]] + xlab("GMF 1") + ylab("GMF 2")
...
excel_supp_fig_3 <- bind_rows(
  lapply(names(MF), function(n) {
    df <- MF[[n]][ , c("PC1", "PC2")] %>% 
      as.data.frame() %>% 
      mutate("Proportion missing" = colMeans(is.na(assay(sce_Petrosius))),
             "method" = n)
    df
  })
)
...
<!--   geom_point(alpha = 0.5)+  scale_colour_viridis() + xlab("PC1") + ggtitle(titles[i])  + -->
<!--   theme_bw() + labs(col = "Proportion missing") -->
<!--   }) -->
...
excel_supp_fig_4 <- df_cor %>% mutate(correlation = abs(correlation))
...
```{r}
Leduc_pca_plot_missing <- lapply(1:9, function(i){
  ggplot(data = MF[[i]], 
       aes(x = PC1,y = PC2, col = colMeans(is.na(assay(sce))))) +
  geom_point(alpha = 0.5)+ scale_colour_gradient2(name = "Proportion \nmissing",
  low = ("red"),
  mid = "grey",
  high = ("blue"),
  midpoint = median(colMeans(is.na(assay(sce)))),
  space = "Lab",
  na.value = "grey50",
  transform = "identity",
  guide = "colourbar",
...
ggplot(data = MF[[i]], 
       aes(x = PC1,y = PC2, col = subcluster, shape = lcBatch)) +
  geom_point(alpha = 0.5) + xlab("PC1") + ggtitle(titles[i]) + 
  theme_bw() + labs(col = "Cell type:   ", shape = "LC-batch:   ") + 
    theme(legend.text = element_text(size = 12), legend.title = element_text(size = 12), legend.box="vertical") +
    scale_colour_manual(name = "Cell type:   ", 
                        values = c("Monocyte    " = "#4269D0FF", "Melanoma A    " = "#E18727FF", "Melanoma B    " = "#20854EFF")) + 
    guides(
    colour = guide_legend(order = 1, override.aes = list(alpha = 1)), 
...
Leduc_pca_plot_batch_correction[[1]] <- Leduc_pca_plot_batch_correction[[1]] + xlab("GMF 1") + ylab("GMF 2")
...
Introduction_Figure <- ggarrange(
  ...
  Leduc_pca_plot_no_batch_correction[[4]]+ theme(legend.position = "bottom",
    ...
    ggtitle("") + guides(col=guide_legend(order = 1, nrow=2,byrow=TRUE, override.aes = list(alpha = 1))),
  ...
  ncol = 3, widths = c(1/3, 1/3, 1/3), align = "hv", labels = "AUTO") + theme(plot.margin = margin(5, 10, 5, 5, "pt"))




Introduction_Figure_2 <- ggarrange(Petrosius_pca_plot_missing[[4]] + theme(legend.position = "bottom")+ ggtitle(""), 
                                   Leduc_pca_plot_missing[[4]]+ theme(legend.position = "bottom")+ ggtitle(""),
                                   CPTAC_pca_plot_missing[[4]]+ theme(legend.position = "bottom")+ ggtitle(""),
                                   ncol = 3, widths = c(1/3, 1/3, 1/3), align = "hv",
                                   labels = "AUTO")+ theme(plot.margin = margin(5, 15, 5, 5, "pt"))
...
ggsave(plot = Introduction_Figure_2, filename = here("Figures/Introduction_Figure_2.pdf"), device = "pdf", dpi = 500, units = "mm", width = 180, height = 90)


excel_fig_1 <- rbind(Petrosius_pca_plot_sampletype[[4]]$data[,c("PC1","PC2")] %>%
                       as.data.frame() %>% 
                       mutate("col" = colData(sce_Petrosius)$SampleType),
                     Leduc_pca_plot_no_batch_correction[[4]]$data[,c("PC1","PC2")] %>%
                       as.data.frame() %>% 
                       mutate("col" = colData(sce)$SampleType),
                     plot_CPTAC[[4]]$data %>%
                       as.data.frame() %>% 
                       mutate("col" = colData(sce_CPTAC_nolab1)$lab))

excel_supp_fig_1 <- rbind(Petrosius_pca_plot_missing[[4]]$data[,c("PC1","PC2")] %>%
                       as.data.frame() %>% 
                       mutate("col" = colMeans(is.na(assay(sce_Petrosius)))),
                     Leduc_pca_plot_missing[[4]]$data[,c("PC1","PC2")] %>%
                       as.data.frame() %>% 
                       mutate("col" = colMeans(is.na(assay(sce)))),
                     CPTAC_pca_plot_missing[[4]]$data%>%
                       as.data.frame() %>% 
                       mutate("col" = colMeans(is.na(assay(sce_CPTAC_nolab1)))))
...
ggsave(plot = Visualisation_Figure, filename = here("Figures/Visualisation_Figure.pdf"), device = "pdf", dpi = 500, units = "mm", width = 180, height = 210)



excel_fig_3 <- rbind(excel_supp_fig_2 %>% 
                       filter(method %in% c("GMF", "CF", "KNN")) %>% 
                       mutate("shape" = NA, "method" = method, "data" = "Petrosius") %>% 
                       rename("Cell Type" = "col"),
                     excel_supp_fig_5 %>% 
                       filter(method %in% c("GMF", "CF", "KNN")) %>% 
                       mutate("data" = "Leduc") %>% 
                       rename("Cell Type" = "col", "lcbatch" = "shape"), 
                     excel_supp_fig_9 %>% 
                       filter(method %in% c("GMF", "CF", "KNN")) %>% 
                       mutate("data" = "CPTAC") %>% 
                       rename("Condition" = "col", "lab" = "shape"),
                     excel_supp_fig_11 %>% 
                       filter(method %in% c("GMF", "CF", "KNN")) %>% 
                       mutate("data" = "CPTAC no lab1") %>% 
                       rename("Condition" = "col", "lab" = "shape"))
...
excel_supp_fig_17 <- Petrosius_imputation_plot$data %>% select(-.group)
excel_supp_fig_19 <- Leduc_imputation_plot$data %>% select(-.group)
...
excel_supp_fig_29 <- rbind(Petrosius_imputation_plot_CV$data %>% 
                             select(-.group) %>% 
                             mutate(data = "Petrosius"),
                           Leduc_imputation_plot_CV$data %>% 
                             select(-.group) %>% 
                             mutate(data = "Leduc"))
...
excel_supp_fig_18 <- Petrosius_imputation_plot_all$data %>% select(-.group)
  
excel_supp_fig_20 <- Leduc_imputation_plot_all$data %>% select(-.group)
...
excel_supp_fig_23 <- CPTAC_all_labs_imputation_plot$data %>% select(-.group)
excel_supp_fig_25 <- CPTAC_nolab1_imputation_plot$data %>% select(-.group)
excel_supp_fig_27 <- CPTAC_only_lab1_imputation_plot$data %>% select(-.group)

excel_supp_fig_24 <- CPTAC_all_labs_imputation_plot_all$data %>% select(-.group)
excel_supp_fig_26 <- CPTAC_nolab1_imputation_plot_all$data %>% select(-.group)
excel_supp_fig_28 <- CPTAC_only_lab1_imputation_plot_all$data %>% select(-.group)
...
```{r}
# Write to supplementary data
library(xlsx)
here()
write.xlsx(df_summarised_Petrosius %>% as.data.frame(), file= here("Figures/Supplementary_Data_MAE.xlsx"), 
           sheetName="MAE_Petrosius", row.names=FALSE)
write.xlsx(df_summarised_Leduc %>% as.data.frame(), file=here("Figures/Supplementary_Data_MAE.xlsx"), 
           sheetName="MAE_Leduc", append=TRUE, row.names=FALSE)
write.xlsx(df_summarised_CPTAC_all %>% as.data.frame(), file=here("Figures/Supplementary_Data_MAE.xlsx"), 
           sheetName="MAE_CPTAC_all", append=TRUE, row.names=FALSE)
write.xlsx(df_summarised_CPTAC_nolab1 %>% as.data.frame(), file=here("Figures/Supplementary_Data_MAE.xlsx"), 
           sheetName="MAE_CPTAC_nolab1", append=TRUE, row.names=FALSE)
write.xlsx(df_summarised_CPTAC_lab1 %>% as.data.frame(), file=here("Figures/Supplementary_Data_MAE.xlsx"), 
           sheetName="MAE_CPTAC_lab1", append=TRUE, row.names=FALSE)
```
...
medians_observed <- data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(missing == FALSE & origin %in% c("Observed")) %>% group_by(condition) %>% summarize(median = median(intensity))
medians_observed <- rbind(medians_observed, medians_observed, medians_observed, 
                          medians_observed, medians_observed, medians_observed)
medians_observed$origin <- factor(rep(c("original", "Imputed_GMF","DAE","VAE", "CF", "KNN"), 
                        each = 5), 
                        levels = c("original", "Imputed_GMF","DAE","VAE", "CF","KNN"),
                        labels = c("Observed", "GMF-imputed","DAE-imputed","VAE-imputed", "CF-imputed","KNN-imputed"))

medians <- data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(((missing == TRUE)|(missing == FALSE & origin %in% c("Observed")))) %>% group_by(condition, origin) %>% 
  summarize(median = median(intensity, na.rm = T))


Imputation_distribution_UPS_nolab1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(((missing == TRUE)|(missing == FALSE & origin %in% c("Observed")))), 
  aes(x = condition, y = intensity, col = condition)) + 
  geom_violin() + 
geom_line(data = medians_observed,
          aes(x = condition, y = median, group = 1),
          inherit.aes = FALSE,
          linetype = "dashed", color = "grey50", size = 0.6) + 
  geom_line(data = medians %>% filter(origin != "Observed"),
          aes(x = condition, y = median, group = 1),
          inherit.aes = FALSE,
  ...
  #ylim(c(-10,10)) + 
  theme_bw() + 
  ylab("log2-normalized intensities") + 
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        legend.title = element_text(size = 14)) + 
    scale_colour_manual(name = "Condition", 
                        values = c("A" = "#BC3C29FF", "B" = "#EFB118FF", "C" = "#3CA951FF",
...
# medians <- data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(((missing == TRUE)|(missing == FALSE & origin %in% c("Observed")))) %>% group_by(condition, origin) %>% 
#   summarize(median = median(intensity, na.rm = T))
# #medians <- rbind(medians, medians, medians, medians, medians, medians)
# 
# Imputation_distribution_UPS_nolab1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(((missing == TRUE)|(missing == FALSE & origin %in% c("Observed")))), 
#   aes(x = condition, y = intensity, col = condition)) + 
#   geom_violin() + 
...
excel_fig_5 <- Imputation_distribution_UPS_nolab1$data
excel_supp_fig_30 <- Imputation_distribution_nonUPS_nolab1$data
...
Distribution_CPTAC_non_UPS_observed_missingness <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][!grepl(rowData(sce[rownames(sce), ])$Proteins,pattern ="UPS")]),] %>% filter(origin %in% c("Observed")) %>% group_by(sample, condition) %>% summarise(mispercent = mean(missing)) , 
  ...
  ylim(c(0,1)) +
  ylab("Proportion of missing values")+
  ggtitle("Proportion of missing values for background yeast peptides")+ 
    scale_colour_manual(name = "Condition", 
                        values = c("A" = "#BC3C29FF", "B" = "#EFB118FF", "C" = "#3CA951FF",
                                   "D" = "#97BBF5FF", "E" = "#FF8AB7FF"))
...
excel_supp_fig_10 <- rbind(
  Distribution_CPTAC_UPS_observed_missingness$data %>% select(-.group) %>% mutate(group = "Human peptides"),
  Distribution_CPTAC_non_UPS_observed_missingness$data %>% select(-.group) %>% mutate(group = "Yeast peptides")
)
```
...
excel_supp_fig_31 <- rbind(imputation_plot_per_lab_UPS$data %>% mutate(group = "Human peptides"),
                           imputation_plot_per_lab_nonUPS$data %>% mutate(group = "Yeast peptides"))
...
excel_fig_4 <- df_cv_all

write.xlsx(df_cv_all, file= here("Figures/Supplementary_Data_Cross_validation.xlsx"), 
           sheetName="Cross-validation", row.names=FALSE)
...
# Put all dataframes into a named list
sheet_list <- list(
  Figure_1 = excel_fig_1,
  Figure_3 = excel_fig_3,
  Figure_4 = excel_fig_4,
  Figure_5 = excel_fig_5,
  Supp_fig_1 = excel_supp_fig_1,
  Supp_fig_2 = excel_supp_fig_2,
  Supp_fig_3 = excel_supp_fig_3,
  Supp_fig_4 = excel_supp_fig_4,
  Supp_fig_5 = excel_supp_fig_5,
  Supp_fig_6 = excel_supp_fig_6,
  Supp_fig_9 = excel_supp_fig_9,
  Supp_fig_10 = excel_supp_fig_10,
  Supp_fig_11 = excel_supp_fig_11,
  Supp_fig_15 = excel_supp_fig_15,
  Supp_fig_17 = excel_supp_fig_17,
  Supp_fig_18 = excel_supp_fig_18,
  Supp_fig_19 = excel_supp_fig_19,
  Supp_fig_20 = excel_supp_fig_20,
  Supp_fig_23 = excel_supp_fig_23,
  Supp_fig_24 = excel_supp_fig_24,
  Supp_fig_25 = excel_supp_fig_25,
  Supp_fig_26 = excel_supp_fig_26,
  Supp_fig_27 = excel_supp_fig_27,
  Supp_fig_28 = excel_supp_fig_28,
  Supp_fig_29 = excel_supp_fig_29,
  Supp_fig_30 = excel_supp_fig_30,
  Supp_fig_31 = excel_supp_fig_31
)

# Loop through sheets
for (sheet_name in names(sheet_list)) {
  addWorksheet(wb, sheet_name)
  writeData(wb, sheet_name, sheet_list[[sheet_name]], rowNames = FALSE)
}
