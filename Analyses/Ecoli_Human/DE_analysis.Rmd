...
VAE <- read.csv(file = here("Analyses/Ecoli_Human/Output/VAE_sce_default.csv"),
                  header = T, row.names = 1)
CF <- read.csv(file = here("Analyses/Ecoli_Human/Output/CF_sce_default.csv"),
                  header = T, row.names = 1)
...
data_all$origin <- factor(rep(c("original", "Imputed_SGD","DAE","VAE", 
                                "CF", "KNN"), 
                        each = nrow(original_assay)), 
                        levels = c("original", "Imputed_SGD","DAE","VAE", 
                                "CF", "KNN"),
                        labels = c("Observed", "SGD-imputed","DAE-imputed","VAE-imputed",
...
data_all$origin <- factor(data_all$origin,
                          levels = c("Observed", "SGD-imputed","DAE-imputed","VAE-imputed",
                                   "CF-imputed","KNN-imputed"),
                        labels = c("Observed", "SGD-imputed","DAE-imputed","VAE-imputed",
                                   "CF-imputed","KNN-imputed"))
...
myColors <- c("black", "red", 
              "blue", "cyan","green", "orchid2")
names(myColors) <- c("Observed", "SGD-imputed","DAE-imputed","VAE-imputed",
                                   "CF-imputed","KNN-imputed")
...
names(myColors) <- c("Observed", "SGD-imputed", 
...
p1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][(rowData(sce[rownames(sce), ])$Proteins %in% 
         ...
         filter(((missing == TRUE & origin %in% c("SGD-imputed","DAE-imputed","VAE-imputed",
...
for (i in contrastNames)
{
    tprFdporiginal <- tprFdp(rowData(pe_original[["msqrobAggregate"]])[[i]]$pval[filter],
             rowData(pe_original[["msqrobAggregate"]])$accession$ecoli[filter],
             rowData(pe_original[["msqrobAggregate"]])[[i]]$adjPval[filter])

tprFdpSGD <- tprFdp(
    rowData(pe_sgd[["msqrobAggregate"]])[[i]]$pval[filter],
    rowData(pe_sgd[["msqrobAggregate"]])$accession$ecoli[filter],
...
hlp <- rbind(
    ...
    hlp$method <- factor(hlp$method, levels = c("SGD","DAE","VAE","CF","KNN", "no-imputation"))
    select <- hlp %>% group_by(method) %>% summarise(select = sum(adjPval < 0.01, na.rm = T))
    
tprFdpPlots_common[[i]] <- hlp %>% filter(method %in% c("SGD", "DAE","VAE","CF","KNN","no-imputation")) %>% 
    ggplot(aes(x = fdp, y = tpr, color = method)) +
    geom_path() + 
    geom_point(data = hlp[c(select$select + nrow(tprFdpSGD)*c(0:(length(unique(hlp$method))-1)))  , ], 
                aes(x = fdp, y = tpr, color = method), cex = 2) +
...
for (i in contrastNames)
{
    ...
    logFC <- data.frame(
        SGD = rowData(pe_sgd[["msqrobAggregate"]])[[i]][filter, 1],
    ...
    ups = rowData(pe_sgd[["msqrobAggregate"]])$accession$ecoli[filter])
...
logFC_plots_common[[i]] <- logFC %>% ggplot(aes(x = method, y = log2FC, fill = ups)) +
    geom_boxplot() +
    geom_hline(yintercept = log2(FCs[[i]]), color = "red") + 
    ggtitle(plot_titles[contrastNames == i]) + 
    theme_bw() + 
    theme(legend.position = "none", axis.text.x = element_text(size = 7.5), axis.title.x = element_blank())  +    
    scale_fill_discrete(name = "Protein", labels = c("Human", "Ecoli spike-in"))
...
plot(full_plot_tprfdp_common)
ggsave(filename = here("Analyses/Ecoli_Human/Output/full_plot_tprfdp_ecoli_common.jpeg"), plot = full_plot_tprfdp_common,
...
ggsave(filename = here("Analyses/Ecoli_Human/Output/full_plot_lfc_ecoli_common.jpeg"), plot = full_plot_lfc_common, width = 4500, 
       unit = "px", height = 3000)
...
p1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][(rowData(sce[rownames(sce), ])$Proteins %in% 
         ...
         filter(((missing == TRUE & origin %in% c("SGD-imputed","DAE-imputed","VAE-imputed",
                                   "CF-imputed","KNN-imputed")|(missing == FALSE & 
                                                                    origin %in% c("Observed"))))),
    aes(x = condition, y = intensity, col = origin)) +
    geom_violin() + 
    theme_bw() + 
    colScale
...
p2 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][(rowData(sce[rownames(sce), ])$Proteins %in% 
         ...
         filter(((missing == TRUE & origin %in% c("SGD-imputed","DAE-imputed","VAE-imputed",
                                   "CF-imputed","KNN-imputed")|(
                                       missing == FALSE & origin %in% c("Observed"))))),
    aes(x = condition, y = intensity, col = origin)) + 
    geom_violin() + 
        theme_bw() + 
    colScale
...
p1 <- ggplot(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][(rowData(sce[rownames(sce), ])$Proteins %in% 
    ...
    facet_wrap(~origin) + 
    geom_violin() + 
        geom_line(aes(x = as.numeric(condition)), y = c(data_all[data_all$peptide %in% rownames(sce[rownames(sce),][(rowData(sce[rownames(sce), ])$Proteins %in% 
            accessions$protein[accessions$ecoli == T])]),]%>% 
         filter(((missing == TRUE & origin %in% c("SGD-imputed","DAE-imputed","VAE-imputed",
                                   "CF-imputed","KNN-imputed")|(missing == FALSE & 
                                                                    origin %in% c("Observed"))))) %>% 
...
```{r}
saveRDS(object = p1, file = "plotp1.RDS")
saveRDS(object = p2, file = "plotp2.RDS")
