...
for x in [100, 1000, 10000, "1e+05"]:
    ...
    with open('Output/time_measurement_VAE_default_' + str(fixed_feature) + '_features_' + str(x) + '.txt', 'w') as file:
        file.write(f"Elapsed time: {elapsed_time} seconds\n")

    
    df_imputed.to_csv('Output/VAE_default_' + str(fixed_feature) + '_features_' + str(x) + '_samples.csv')
